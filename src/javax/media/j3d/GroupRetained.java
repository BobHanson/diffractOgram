...
// per shared path ordered path data
	...
	/**
	 * If collisionBound is set, this is equal to the transformed collisionBounds,
	 * otherwise it is equal to the transformed localBounds. This variable is set to
	 * null unless collisionTarget = true. This bound is only used by mirror Group.
	 */
	BoundingBox collisionVwcBounds;

	/**
	 * Mirror group of this node, it is only used when collisionTarget = true.
	 * Otherwise it is set to null. If not in shared group, only entry 0 is used.
	 *
	 */
...
// higher level VSGs
	...
	void setCollisionBounds(Bounds bounds) {
		...
		if (source.isLive()) {
			...
			message.threads = J3dThread.UPDATE_TRANSFORM | J3dThread.UPDATE_GEOMETRY;
	...
	/**
	 * Removes the specified child from this Group's list of children. If the
	 * specified child is not found, the method returns quietly
	 *
	...
	void checkValidChild(Node child, String s) {
		...
		if ((child != null)
				&& (((child instanceof BranchGroup) && (((BranchGroupRetained) child.retained).attachedToLocale))
	...
	void moveTo(BranchGroup bg) {
		...
		if (this.source.isLive()) {
			...
			synchronized (universe.sceneGraphLock) {
				...
				universe.setLiveState.clear();
				if (oldParent == null)
					universe.notifyStructureChangeListeners(((BranchGroupRetained) bg.retained).locale, this.source,
							bg);
				else
	...
	void removeLight(int numLgt, LightRetained[] removelight, HashKey key) {
		...
		/*
		 * // XXXX: lights may remove twice or more during clearLive(), // one from
		 * itself and one call from every LightRetained // reference this. So there is
		 * case that this procedure get // called when light already removed. if (i >=
		 * 0) lights.remove(i);
		 */
	}
			...
			for (int i = children.size() - 1; i >= 0; i--) {
				...
				if (child != null) {
					if (child instanceof GroupRetained && (child.source.isLive() || child.isInSetLive()))
						...
						((GroupRetained) (ln.sharedGroup)).processAllNodesForScopedLight(numLgts, ml, list,
	...
	void processRemoveAllNodesForScopedLight(int numLgts, LightRetained[] ml, ArrayList list, HashKey k) {
		...
		if (this.source.isLive() && !isInClearLive) {
			for (int i = children.size() - 1; i >= 0; i--) {
				NodeRetained child = children.get(i);
				if (child != null) {
					if (child instanceof GroupRetained
							&& (child.source.isLive() && !((GroupRetained) child).isInClearLive))
						...
						((GroupRetained) (ln.sharedGroup)).processRemoveAllNodesForScopedLight(numLgts, ml, list,
								k.append("+").append(ln.nodeId));
						k.count = lastCount;
					} else if (child instanceof Shape3DRetained && child.source.isLive()) {
						((Shape3DRetained) child).getMirrorObjects(list, k);
			...
			for (int i = children.size() - 1; i >= 0; i--) {
				...
				if (child != null) {
					if (child instanceof GroupRetained && (child.source.isLive() || child.isInSetLive()))
						...
						((GroupRetained) (ln.sharedGroup)).processAddNodesForScopedFog(mfog, list,
								k.append("+").append(ln.nodeId));
						k.count = lastCount;
					} else if (child instanceof Shape3DRetained && child.source.isLive()) {
						((Shape3DRetained) child).getMirrorObjects(list, k);
			...
			for (int i = children.size() - 1; i >= 0; i--) {
				...
				if (child != null) {
					if (child instanceof GroupRetained
						...
						((GroupRetained) (ln.sharedGroup)).processRemoveAllNodesForScopedFog(mfog, list,
								k.append("+").append(ln.nodeId));
						k.count = lastCount;
					} else if (child instanceof Shape3DRetained && child.source.isLive()) {
						((Shape3DRetained) child).getMirrorObjects(list, k);
			...
			for (int i = children.size() - 1; i >= 0; i--) {
				...
				if (child != null) {
					if (child instanceof GroupRetained && (child.source.isLive() || child.isInSetLive()))
						...
						((GroupRetained) (ln.sharedGroup)).processAddNodesForScopedModelClip(mModelClip, list,
								k.append("+").append(ln.nodeId));
						k.count = lastCount;
					} else if (child instanceof Shape3DRetained && child.source.isLive()) {
						((Shape3DRetained) child).getMirrorObjects(list, k);
			...
			for (int i = children.size() - 1; i >= 0; i--) {
				...
				if (child != null) {
					if (child instanceof GroupRetained
						...
						((GroupRetained) (ln.sharedGroup)).processRemoveAllNodesForScopedModelClip(mModelClip, list,
								k.append("+").append(ln.nodeId));
						k.count = lastCount;
					} else if (child instanceof Shape3DRetained && child.source.isLive()) {
						((Shape3DRetained) child).getMirrorObjects(list, k);
			...
			for (int i = children.size() - 1; i >= 0; i--) {
				...
				if (child != null) {
					if (child instanceof GroupRetained && (child.source.isLive() || child.isInSetLive()))
						...
						((GroupRetained) (ln.sharedGroup)).processAddNodesForScopedAltApp(mAltApp, list,
								k.append("+").append(ln.nodeId));
						k.count = lastCount;
					} else if (child instanceof Shape3DRetained && child.source.isLive()) {
						((Shape3DRetained) child).getMirrorObjects(list, k);
			...
			for (int i = children.size() - 1; i >= 0; i--) {
				...
				if (child != null) {
					if (child instanceof GroupRetained
						...
						((GroupRetained) (ln.sharedGroup)).processRemoveNodesForScopedAltApp(mAltApp, list,
								k.append("+").append(ln.nodeId));
						k.count = lastCount;
					} else if (child instanceof Shape3DRetained && child.source.isLive()) {
						((Shape3DRetained) child).getMirrorObjects(list, k);
	...
	void setAlternateCollisionTarget(boolean target) {
		...
		if (source.isLive()) {
			...
			if (target) {
				...
				TargetsInterface ti = getClosestTargetsInterface(TargetsInterface.TRANSFORM_TARGETS);
				if (ti != null) {
					...
					for (int i = 0; i < numPath; i++) {
						...
						if (ct != null) {
							targets.addNode(mirrorGroup.get(i), Targets.GRP_TARGETS);
					...
					ti.updateTargetThreads(TargetsInterface.TRANSFORM_TARGETS, newCtArr);
					ti.resetCachedTargets(TargetsInterface.TRANSFORM_TARGETS, newCtArr, -1);
				}
	 ...
	 * info and calls it.
	 */
	void checkSetLive(NodeRetained child, int childIndex, J3dMessage messages[], int messageIndex,
			NodeRetained linkNode) {
		checkSetLive(child, childIndex, localToVworldKeys, inSharedGroup, messages, messageIndex, linkNode);
	}
	 ...
	 * info and calls it.
	 */
	void checkSetLive(NodeRetained child, int childIndex, HashKey keys[], boolean isShared, J3dMessage messages[],
	...
	void checkClearLive(NodeRetained child, J3dMessage messages[], int messageIndex, int childIndex,
			NodeRetained linkNode) {
		checkClearLive(child, localToVworldKeys, inSharedGroup, messages, messageIndex, childIndex, linkNode);
	}

	/**
	 * This checks if clearLive needs to be called. If it does, it gets the needed
	 * info and calls it.
	 */
	void checkClearLive(NodeRetained child, HashKey keys[], boolean isShared, J3dMessage messages[], int messageIndex,
			...
			if (isSetLive) {
				...
				if (!inSharedGroup) {
					if (s.transformTargets[0] != null) {
						ct = ti.getCachedTargets(TargetsInterface.TRANSFORM_TARGETS, 0, -1);
						if (ct != null) {
	...
	TargetsInterface initSwitchStates(SetLiveState s, NodeRetained parentNode, NodeRetained childNode,
		...
		TargetsInterface ti = getClosestTargetsInterface(TargetsInterface.SWITCH_TARGETS);
		if (ti != null) {
		...
		if (isSetLive) {
			...
			if (ti != null) {
				s.switchStates = ti.getTargetsData(TargetsInterface.SWITCH_TARGETS, parentSwitchLinkChildIndex);
			} else {
			...
			if (isSetLive) {
				...
				if (!inSharedGroup) {
					...
					if (s.switchTargets[0] != null) {
						ct = ti.getCachedTargets(TargetsInterface.SWITCH_TARGETS, 0, parentSwitchLinkChildIndex);
						if (ct != null) {
					...
					for (int i = 0; i < s.keys.length; i++) {
						if (s.switchTargets[i] != null) {
							ct = ti.getCachedTargets(TargetsInterface.SWITCH_TARGETS, i, parentSwitchLinkChildIndex);
							if (ct != null) {
				...
				ti.resetCachedTargets(TargetsInterface.SWITCH_TARGETS, newCtArr, parentSwitchLinkChildIndex);
				...
				CachedTargets[] newCtArr = new CachedTargets[localToVworld.length];
	...
	TargetsInterface getClosestTargetsInterface(int type) {
		return (type == TargetsInterface.TRANSFORM_TARGETS) ? (TargetsInterface) parentTransformLink
	...
	void processCollisionTarget(SetLiveState s) {
		...
		Bounds bound = (collisionBound != null ? collisionBound : getEffectiveBounds());
		if (inSharedGroup) {
			for (int i = 0; i < s.keys.length; i++) {
				int j;
				g = new GroupRetained();
				...
				j = s.keys[i].equals(localToVworldKeys, 0, localToVworldKeys.length);
				if (j < 0) {
				...
				mirrorGroup.add(g);
				/*
				 * System.err.println("processCollisionTarget mirrorGroup.add() : " + g.getId()
				 * + " mirrorGroup.size() " + mirrorGroup.size());
				 */
	...
	boolean isStaticChildren() {
		if (source.getCapability(Group.ALLOW_CHILDREN_READ) || source.getCapability(Group.ALLOW_CHILDREN_WRITE)) {
			return false;
		}
	...
	void compile(CompileState compState) {
		...
		if (isRoot || this.usedInScoping() || (parent instanceof SwitchRetained)) {
			mergeFlag = SceneGraphObjectRetained.DONT_MERGE;
		}
...
//	HashKey[] savedLocalToVworldKeys = localToVworldKeys;
		...
		if (savedParentFogs != null) {
			if (allocatedFogs) {
				if (inSharedGroup) {
					...
					// BH This did not do what the author thinks it does
					ArrayList<FogRetained> fogs = this.fogs.get(0);
					size = l.size();
					for (int m = 0; m < size; m++) {
						fogs.remove(l.get(m));
					}
		...
		if (savedParentMclips != null) {
			if (allocatedMclips) {
				if (inSharedGroup) {
					...
					// BH This did not do what the author thinks it does
					ArrayList<ModelClipRetained> modelClips = this.modelClips.get(0);
					size = l.size();
					// BH This does not do what the author thinks it does
					for (int m = 0; m < size; m++) {
		...
		if (savedParentAltApps != null) {
			if (allocatedAltApps) {
				if (inSharedGroup) {
					...
					// BH This did not do what the author thinks it does
					ArrayList<AlternateAppearanceRetained> altAppearances = this.altAppearances.get(0);
					size = l.size();
					for (int m = 0; m < size; m++) {
				...
				for (i = s.keys.length - 1; i >= 0; i--) {
					...
					for (int j = mirrorGroup.size() - 1; j >= 0; j--) {
						...
						if (g.key.equals(hkey)) {
							...
							if (s.transformTargets != null && s.transformTargets[j] != null) {
	...
	/**
	 * Return true of nodeR is not under a switch group or nodeR is enable under a
	 * switch group.
	 */
	static boolean isNodeSwitchOn(NodeRetained node, HashKey key) {
		...
		synchronized (node.universe.sceneGraphLock) {
			do {
				if ((node instanceof SwitchRetained) && (prevNode != null)
	...
	static boolean validSwitchChild(SwitchRetained sw, NodeRetained node) {
		...
		// Switch.CHILD_MASK
		for (int i = children.size() - 1; i >= 0; i--) {
			if (sw.childMask.get(i) && (children.get(i) == node)) {
				return true;
			}
		}
		return false;
	...
	/**
	 * Create mirror group when this Group AlternateCollisionTarget is set to true
	 * while live.
	 */
	...
	void setBoundsAutoCompute(boolean autoCompute) {
		if (autoCompute != boundsAutoCompute) {
			...
			if (source.isLive() && collisionBound == null && autoCompute && mirrorGroup != null) {
	...
	void setBounds(Bounds bounds) {
		...
		if (source.isLive() && !boundsAutoCompute && collisionBound == null && mirrorGroup != null) {
			...
			for (int i = 0; i < nchildren; i++) {
				...
				if (child instanceof LeafRetained) {
					if (child instanceof LinkRetained) {
						...
						keyList = ((GroupRetained) (ln.sharedGroup)).processViewSpecificInfo(mode,
								k.append("+").append(ln.nodeId), v, vsgList, keyList, leafList);
						k.count = lastCount;
					} else {
	...
	void findSwitchInfo(SetLiveState s, NodeRetained parentNode, NodeRetained childNode, NodeRetained linkNode) {
		...
		if (!inSharedGroup) {
			...
			while (parent != null) {
				if (parent instanceof SwitchRetained) {
					...
					if (s.closestSwitchParents[0] == null) {
						...
						s.closestSwitchIndices[0] = ((SwitchRetained) parent).switchIndexCount++;
					}
					if (parentSwitchLinkChildIndex == -1) {
						parentSwitchLinkChildIndex = ((GroupRetained) parent).children.indexOf(child);
					}
			...
			s.closestSwitchParents = new SwitchRetained[localToVworldKeys.length];
				...
				while (parent != null) {
					...
					if (parent instanceof SwitchRetained) {
						...
						if (s.closestSwitchParents[i] == null) {
							...
							s.closestSwitchIndices[i] = ((SwitchRetained) parent).switchIndexCount++;
