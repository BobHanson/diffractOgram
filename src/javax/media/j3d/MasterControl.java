...
class MasterControl {
	 ...
	 * by MIK OF CLASSX
	 *
	 * the flag to indicate whether the background of the offscreen canvas must be
	 * transparent or not false by default
	 */
	boolean transparentOffScreen = false;

	/**
	 * Flag to indicate whether Pbuffers are used for off-screen rendering; true by
	 * default. Set by the "j3d.usePbuffer" property, When this flag is set to
	 * false, Bitmap (Windows) or Pixmap (UNIX) rendering will be used
	 */
	boolean usePbuffer = true;

	/**
	 * Flag to indicate whether should renderer view frustum culling is done; true
	 * by default. Set by the -Dj3d.viewFrustumCulling property, When this flag is
	...
	/**
	 * Markers for the last transform structure update thread and the last update
	 * thread.
	 */
	 ...
	 * It is initialized once, the first time that a MasterControl object is
	 * created.
	 */
	...
	// Issue 249 - flag that indicates whether the soleUser optimization is
	// permitted
	...
	Object rendererCleanupArgs[] = { new Integer(Renderer.REQUESTCLEANUP), null, null };
	...
	/**
	 * Constructs a new MasterControl object. Note that there is exatly one
	 * MasterControl object, created statically by VirtualUniverse.
	 */
	MasterControl() {
		...
		if (J3dDebug.devPhase) {
			...
			J3dDebug.debug = getBooleanProperty("j3d.debug", false, "J3dDebug.debug");
		}

		// Check to see whether shared contexts are allowed
		isSharedCtx = getBooleanProperty("j3d.sharedctx", isSharedCtx, "shared contexts");

		doCompaction = getBooleanProperty("j3d.docompaction", doCompaction, "compaction");

		// by MIK OF CLASSX
		transparentOffScreen = getBooleanProperty("j3d.transparentOffScreen", transparentOffScreen,
				"transparent OffScreen");

		usePbuffer = getBooleanProperty("j3d.usePbuffer", usePbuffer, "Off-screen Pbuffer");

		viewFrustumCulling = getBooleanProperty("j3d.viewFrustumCulling", viewFrustumCulling,
				"View frustum culling in the renderer is");

		sortShape3DBounds = getBooleanProperty("j3d.sortShape3DBounds", sortShape3DBounds,
				"Shape3D bounds enabled for transparency sorting", "Shape3D bounds *ignored* for transparency sorting");

		forceReleaseView = getBooleanProperty("j3d.forceReleaseView", forceReleaseView,
...
//					  "NV_register_combiners disabled");
		...
		isDisplayList = getBooleanProperty("j3d.displaylist", isDisplayList, "display list");

		implicitAntialiasing = getBooleanProperty("j3d.implicitAntialiasing", implicitAntialiasing,
				"implicit antialiasing");

		isCompiledVertexArray = getBooleanProperty("j3d.compiledVertexArray", isCompiledVertexArray,
				"compiled vertex array");

		boolean j3dOptimizeSpace = getBooleanProperty("j3d.optimizeForSpace", true, "optimize for space");
		...
		doDsiRenderLock = getBooleanProperty("j3d.renderLock", doDsiRenderLock, "render lock");

		// Check to see whether we enforce power-of-two textures
		enforcePowerOfTwo = getBooleanProperty("j3d.textureEnforcePowerOfTwo", enforcePowerOfTwo,
				"checking power-of-two textures");

		// Issue 249 - check to see whether the soleUser optimization is permitted
		allowSoleUser = getBooleanProperty("j3d.allowSoleUser", allowSoleUser, "sole-user mode");

		// Issue 266 - check to see whether null graphics configs are allowed
		allowNullGraphicsConfig = getBooleanProperty("j3d.allowNullGraphicsConfig", allowNullGraphicsConfig,
				"null graphics configs");

		// Issue 239 - check to see whether per-frame stencil clear is enabled
		stencilClear = getBooleanProperty("j3d.stencilClear", stencilClear, "per-frame stencil clear");

		// Check to see if stereo mode is sharing the Z-buffer for both eyes.
		sharedStereoZBuffer = getBooleanProperty("j3d.sharedstereozbuffer", sharedStereoZBuffer,
		...
		Integer threadLimit = java.security.AccessController
				.doPrivileged(new java.security.PrivilegedAction<Integer>() {
					@Override
					public Integer run() {
		...
		if (J3dDebug.debug || cpuLimit != defaultThreadLimit) {
			System.err.println("Java 3D: concurrent threadLimit = " + cpuLimit);
		}

		// Get the input device scheduler sampling time
		Integer samplingTime = java.security.AccessController
				.doPrivileged(new java.security.PrivilegedAction<Integer>() {
					@Override
					public Integer run() {
		...
		if (samplingTime.intValue() > 0) {
			InputDeviceScheduler.samplingTime = samplingTime.intValue();
		...
		Integer vattrOffset = java.security.AccessController
				.doPrivileged(new java.security.PrivilegedAction<Integer>() {
					@Override
					public Integer run() {
		...
		if (J3dDebug.debug || glslVertexAttrOffset != defaultGLSLVertexAttrOffset) {
			System.err.println("Java 3D: glslVertexAttrOffset = " + glslVertexAttrOffset);
		}

		// Issue 480 : Cache bounds returned by getBounds()
		cacheAutoComputedBounds = getBooleanProperty("j3d.cacheAutoComputeBounds", cacheAutoComputedBounds,
				"Cache AutoCompute Bounds, accelerates getBounds()");

		// Issue 544
		useBoxForGroupBounds = getBooleanProperty("j3d.useBoxForGroupBounds", useBoxForGroupBounds,
				"Use of BoundingBox for group geometric bounds");

		// Check for obsolete properties
		String[] obsoleteProps = { "j3d.backgroundtexture", "j3d.forceNormalized", "j3d.g2ddrawpixel",
				"j3d.simulatedMultiTexture", "j3d.useFreeLists", };
	 ...
	 *
	 * WARNING - for probable incorrect or inconsistent api usage INFO - for
	 * informational messages such as performance hints (less verbose than FINE)
	 * FINE - for informational messages from inner loops FINER - using default
	 * values which may not be optimal
	 */
	 ...
	 *
	 * WARNING - statistical anomalies INFO - basic performance stats - not too
	 * verbose and minimally intrusive FINE - somewhat verbose and intrusive FINER -
	 * more verbose and intrusive FINEST - most verbose and intrusive
	 */
	 ...
	 *
	 * SEVERE - Serious internal errors WARNING - Possible internal errors or
	 * anomalies INFO - General informational messages FINE - Internal debugging
	 * information - somewhat verbose FINER - Internal debugging information - more
	 * verbose FINEST - Internal debugging information - most verbose
	 */
	...
	static boolean getBooleanProperty(String prop, boolean defaultValue, String msg) {
		return getBooleanProperty(prop, defaultValue, (msg + " enabled"), (msg + " disabled"));
	}

	/**
	 * Method to create and initialize the rendering Pipeline object, and to load
	 * the native libraries needed by Java 3D. This is called by the static
	 * initializer in VirtualUniverse <i>before</i> the MasterControl object is
	 * created.
	 */
	static void loadLibraries() {
		...
		Pipeline.Type pipelineType = (/** @j2sNative 1 ? null : */
		Pipeline.Type.JOGL);
	...
	InputDeviceBlockingThread getInputDeviceBlockingThread(final InputDevice device) {
		...
		return java.security.AccessController
				.doPrivileged(new java.security.PrivilegedAction<InputDeviceBlockingThread>() {
					@Override
					public InputDeviceBlockingThread run() {
						synchronized (rootThreadGroup) {
							InputDeviceBlockingThread thread = new InputDeviceBlockingThread(rootThreadGroup, device);
	...
	/**
	 * This returns a context creation time stamp Note: this has to be called under
	 * the contextCreationLock
	 */
	...
	/**
	 * Create a Renderer if it is not already done so. This is used for
	 * GraphicsConfigTemplate3D passing graphics call to RequestRenderer, and for
	 * creating an off-screen buffer for an off-screen Canvas3D.
	 */
	...
	void postRequest(Integer type, Object obj) {
		...
		synchronized (mcThreadLock) {
			synchronized (requestObjList) {
				if (mcThread == null) {
					if ((type == ACTIVATE_VIEW) || (type == GETBESTCONFIG) || (type == SET_VIEW)
							|| (type == ISCONFIGSUPPORT) || (type == SET_QUERYPROPERTIES)
	...
	/**
	 * This procedure is invoked when isRunning is false. Return true when there is
	 * no more pending request so that Thread can terminate. Otherwise we have to
	 * recreate the MC related threads.
	 */
	...
	/**
	 * This method increments and returns the next time value timeLock must get
	...
	/**
	 * This takes a given message and parses it out to the structures and marks its
	 * time value.
	 */
	 ...
	 * marks the time value. Make sure, setWork() is done at the very end to make
	 * sure all the messages will be processed in the same frame
	 */
	...
	void createMasterControlThread() {
		...
		java.security.AccessController.doPrivileged(new java.security.PrivilegedAction<Object>() {
			...
			public Object run() {
				synchronized (rootThreadGroup) {
					mcThread = new MasterControlThread(rootThreadGroup);
	...
	/**
	 * Send a message to another Java 3D thread. This variant is only call by
	 * TimerThread for Input Device Scheduler or to redraw all View for RenderThread
	 */
		...
		synchronized (timeLock) {
			...
			if ((targetThreads & J3dThread.INPUT_DEVICE_SCHEDULER) != 0) {
				synchronized (inputDeviceThreads) {
					InputDeviceScheduler ds[] = (InputDeviceScheduler[]) inputDeviceThreads.toArray(false);
					for (int i = inputDeviceThreads.size() - 1; i >= 0; i--) {
						if (ds[i].physicalEnv.activeViewRef > 0) {
							ds[i].getThreadData().lastUpdateTime = time;
						}
					}
			...
			if ((targetThreads & J3dThread.RENDER_THREAD) != 0) {
				synchronized (renderThreadData) {
					J3dThreadData[] threads = (J3dThreadData[]) renderThreadData.toArray(false);
	...
	/**
	 * Send a message to another Java 3D thread. This variant is only call by
	 * TimerThread for Sound Scheduler
	 */
	...
	/**
	 * Send a message to another Java 3D thread. This variant is only called to
			...
			if ((targetThreads & J3dThread.SOUND_SCHEDULER) != 0) {
				synchronized (views) {
					...
					for (int i = views.arraySize() - 1; i >= 0; i--) {
						if ((v[i].soundScheduler != null) && (v[i].universe == u)) {
	...
	/**
	 * Return a clone of View, we can't access individual element of View after
	 * getting the size in separate API call without synchronized views.
	 */
	...
	/**
	 * This snapshots the time values to be used for this iteration. Note that this
	 * method is called without the timeLock held. We must synchronize on timeLock
	 * to prevent updating thread.lastUpdateTime from user thread in sendMessage()
	 * or sendRunMessage().
	 */
	private void updateTimeValues() {
		synchronized (timeLock) {
			...
			J3dThreadData threads[] = (J3dThreadData[]) stateWorkThreads.toArray(false);
			int size = stateWorkThreads.arraySize();

			while (i < lastTransformStructureThread) {
				thread = threads[i++];

				if ((thread.lastUpdateTime > thread.lastRunTime) && !thread.thread.userStop) {
					lastThread = thread;
				...
				if ((thread.lastUpdateTime > thread.lastRunTime) && !thread.thread.userStop) {
					...
					if (thread.thread.lastWaitTimestamp == waitTimestamp) {
						...
						if (thread.view != v) {
							...
							v = thread.view;
							threadToRun = true;
							lastRunThread.threadOpts = (J3dThreadData.STOP_TIMER | J3dThreadData.WAIT_ALL_THREADS);
							((Object[]) lastRunThread.threadArgs)[3] = lastRunThread.view;
							thread.threadOpts = (J3dThreadData.START_TIMER | J3dThreadData.CONT_THREAD);
						} else {
							if ((lastRunThread.threadOpts & J3dThreadData.START_TIMER) != 0) {
								lastRunThread.threadOpts = (J3dThreadData.START_TIMER | J3dThreadData.WAIT_ALL_THREADS);

							} else {
								lastRunThread.threadOpts = J3dThreadData.WAIT_ALL_THREADS;
							}
							thread.threadOpts = J3dThreadData.CONT_THREAD;
	...
	private void createUpdateThread(J3dStructure structure) {
		...
		if (s.updateThread == null) {
			...
			s.threadData.lastUpdateTime = Math.max(currentTime, s.threadData.lastUpdateTime);
		}
	}
		...
		if (structure != null) {
			...
			synchronized (mlist) {
				...
				if (size > 0) {
					...
					while (i < size) {
						m = mess[i];
						if ((v == null) || (m.view == v) || ((m.view == null) && !otherViewExist)) {
	...
	/**
	 * This register a View with MasterControl. The View has at least one Canvas3D
	...
	private void registerView(View v) {
		...
		if (univ.behaviorScheduler == null) {
			...
			threadData.lastUpdateTime = Math.max(currentTime, threadData.lastUpdateTime);
		}
	...
	/**
	 * This unregister a View with MasterControl. The View no longer has any
	 * Canvas3Ds in a container.
	 */
	private void unregisterView(View v) {
		...
		for (Enumeration<PhysicalEnvironment> e = PhysicalEnvironment.physicalEnvMap.keys(); e.hasMoreElements();) {
			...
			if (!phyEnvHasUser) {
				if (J3dDebug.devPhase) {
					J3dDebug.doDebug(J3dDebug.masterControl, J3dDebug.LEVEL_1,
							"MC: Destroy InputDeviceScheduler thread " + sched);
				}
				sched.finish();
		...
		if (views.isEmpty()) {
			if (J3dDebug.devPhase) {
				J3dDebug.doDebug(J3dDebug.masterControl, J3dDebug.LEVEL_1, "MC: Destroy all Renderers");
			}
			// remove all Renderers if this is the last View
			for (Enumeration<Renderer> e = Screen3D.deviceRendererMap.elements(); e.hasMoreElements();) {
				...
				if (scr != null) {
					if (scr.renderer != null) {
						rendererCleanupArgs[2] = REMOVEALLCTXS_CLEANUP;
						runMonitor(RUN_RENDERER_CLEANUP, null, null, null, scr.renderer);
						scr.renderer = null;
					}
	...
	/**
	 * Destroy all VirtualUniverse related threads. This procedure may call two
	 * times when Locale detach in a live viewPlatform.
	 */
	private void destroyUniverseThreads(VirtualUniverse univ) {

		if (regUniverseList.contains(univ)) {
			if (J3dDebug.devPhase) {
				J3dDebug.doDebug(J3dDebug.masterControl, J3dDebug.LEVEL_1, "MC: Destroy universe threads " + univ);
			}
	...
	/**
	 * Note that we have to go through all views instead of evaluate only the canvas
	 * in a single view since each screen may share by multiple view
	 */
					...
					for (int k = canvasList[j].length - 1; k >= 0; k--) {
						...
						if (cv.active) {
							...
							if (!added) {
								...
								J3dThreadData renderData = screen.renderer.getThreadData(v, null);
								renderThreadData.add(renderData);

								// only if renderBin is ready then we
								// update the lastUpdateTime to make it run
								if (v.renderBinReady) {
									renderData.lastUpdateTime = Math.max(currentTime, renderData.lastUpdateTime);
								}
								added = true;
							}
							// Renderer message data thread
							J3dThreadData renderData = screen.renderer.getThreadData(v, cv);
							renderThreadData.add(renderData);
							if (v.renderBinReady) {
								renderData.lastUpdateTime = Math.max(currentTime, renderData.lastUpdateTime);
							}
	...
	private void assignNewPrimaryView(VirtualUniverse univ) {
		...
		for (int i = 0; i < nviews; i++) {
			View view = v[i];
			if (view.active && view.isRunning && (univ == view.universe)) {
	...
	RenderMethod getCompressedGeometryRenderMethod() {
		if (compressedGeometryRenderMethod == null) {
			compressedGeometryRenderMethod = new CompressedGeometryRenderMethod();
		}
			...
			for (int k = canvasList[j].length - 1; k >= 0; k--) {
				...
				if (!cv.validCanvas) {
					if ((cv.screen != null) && (cv.screen.renderer != null)) {
						...
						runMonitor(RUN_RENDERER_CLEANUP, null, null, null, cv.screen.renderer);
	...
	/**
	 * This fun little method does all the hard work of setting up the work thread
	 * list.
	 */
	private void updateWorkThreads() {
		...
		VirtualUniverse universes[] = (VirtualUniverse[]) activeUniverseList.toArray(false);
		...
		for (i = size - 1; i >= 0; i--) {
			addToStateThreads(universes[i].behaviorScheduler.getThreadData(null, null));
		}

		// Now InputDeviceScheduler

		InputDeviceScheduler ds[] = (InputDeviceScheduler[]) inputDeviceThreads.toArray(true);
	...
	/**
	 * A convienence wrapper function for various parts of the system to force MC to
	 * run.
	 */
	final void setWork() {
	...
	/**
	 * Call from GraphicsConfigTemplate to evaluate current capabilities using
	 * Renderer thread to invoke native graphics library functions. This avoid
	 * MT-safe problem when using thread directly invoke graphics functions.
	 */
	void sendRenderMessage(GraphicsConfiguration gc, Object arg, Integer mtype) {
	...
	void doWork() {
		...
		for (int i = views.size() - 1; i >= 0; i--) {
			if (v[i].active) {
				...
				if ((v[i].viewCache.vcDirtyMask != 0 && !v[i].renderBin.orientedRAs.isEmpty())
	...
	private void handlePendingRequest() {
		...
		for (int i = 0; i < size; i++) {
			...
			if (type == RESET_CANVAS) {
				...
				GraphicsConfiguration gc = ((GraphicsConfiguration[]) ((GraphicsConfigTemplate3D) o).testCfg)[0];
				sendRenderMessage(gc, o, type);
				rendererRun = true;
			} else if (type == ISCONFIGSUPPORT) {
				GraphicsConfiguration gc = (GraphicsConfiguration) ((GraphicsConfigTemplate3D) o).testCfg;
				sendRenderMessage(gc, o, type);
				rendererRun = true;
			} else if ((type == SET_GRAPHICSCONFIG_FEATURES) || (type == SET_QUERYPROPERTIES)) {
					...
					if (!views.contains(v)) {
						...
						if (v.resetUnivCount == v.universeCount) {
							...
							if (running == false) {
								...
								/*
								 * // Don't free list cause there may // have some other thread returning ID //
								 * after it. FreeListManager.clearList(FreeListManager.DISPLAYLIST);
								 * FreeListManager.clearList(FreeListManager.TEXTURE2D);
								 * FreeListManager.clearList(FreeListManager.TEXTURE3D);
								 * 
								 * synchronized (textureIdLock) { textureIdCount = 0; }
								 */
							}
						}
					}
	...
	synchronized void runMonitor(int action, UnorderList stateThreadList, UnorderList renderThreadList,
			UnorderList requestRenderThreadList, J3dThread nthread) {
		...
		case RUN_THREADS:
			...
			while (!done) {
				...
				while (!renderWaiting && currentRenderThread != renderThreadSize) {
					...
					if (cpuLimit == 1) {
						...
						threadPending++;
						thread.thread.runMonitor(J3dThread.RUN, currentTime, (Object[]) thread.threadArgs);
					}
					...
					if ((cpuLimit != 1) && (thread.threadOpts & J3dThreadData.WAIT_ALL_THREADS) != 0) {
				...
				while (!stateWaiting && currentStateThread != stateThreadSize) {
				...
				if (!renderWaiting && (currentRenderThread == renderThreadSize)) {
					currentRequestRenderThread = 0;
					while (!renderWaiting && (currentRequestRenderThread != requestRenderThreadSize)) {

						thread = requestRenderThreads[currentRequestRenderThread++];
				...
				if (cpuLimit != 1) {
					if ((renderWaiting && (currentStateThread == stateThreadSize))
							|| (stateWaiting && currentRenderThread == renderThreadSize)
				...
				if ((currentStateThread == stateThreadSize) && (currentRenderThread == renderThreadSize)
						&& (currentRequestRenderThread == requestRenderThreadSize) && (threadPending == 0)) {
		...
		case SET_WORK_FOR_REQUEST_RENDERER:
			...
			workToDo = true;
			if (state == WAITING_FOR_CPU || state == WAITING_FOR_THREADS || state == SLEEPING) {
				notify();
			}
			break;

		case RUN_RENDERER_CLEANUP:
			nthread.runMonitor(J3dThread.RUN, currentTime, rendererCleanupArgs);
	...
	static String mtype[] = { "INSERT_NODES", "REMOVE_NODES", "RUN", "TRANSFORM_CHANGED", "UPDATE_VIEW", "STOP_THREAD",
			"COLORINGATTRIBUTES_CHANGED", "LINEATTRIBUTES_CHANGED", "POINTATTRIBUTES_CHANGED",
			"POLYGONATTRIBUTES_CHANGED", "RENDERINGATTRIBUTES_CHANGED", "TEXTUREATTRIBUTES_CHANGED",
			"TRANSPARENCYATTRIBUTES_CHANGED", "MATERIAL_CHANGED", "TEXCOORDGENERATION_CHANGED", "TEXTURE_CHANGED",
			"MORPH_CHANGED", "GEOMETRY_CHANGED", "APPEARANCE_CHANGED", "LIGHT_CHANGED", "BACKGROUND_CHANGED",
			"CLIP_CHANGED", "FOG_CHANGED", "BOUNDINGLEAF_CHANGED", "SHAPE3D_CHANGED", "TEXT3D_TRANSFORM_CHANGED",
			"TEXT3D_DATA_CHANGED", "SWITCH_CHANGED", "COND_MET", "BEHAVIOR_ENABLE", "BEHAVIOR_DISABLE",
			"INSERT_RENDERATOMS", "ORDERED_GROUP_INSERTED", "ORDERED_GROUP_REMOVED", "COLLISION_BOUND_CHANGED",
			"REGION_BOUND_CHANGED", "MODELCLIP_CHANGED", "BOUNDS_AUTO_COMPUTE_CHANGED", "SOUND_ATTRIB_CHANGED",
			"AURALATTRIBUTES_CHANGED", "SOUNDSCAPE_CHANGED", "ALTERNATEAPPEARANCE_CHANGED", "RENDER_OFFSCREEN",
			"RENDER_RETAINED", "RENDER_IMMEDIATE", "SOUND_STATE_CHANGED", "ORIENTEDSHAPE3D_CHANGED",
			"TEXTURE_UNIT_STATE_CHANGED", "UPDATE_VIEWPLATFORM", "BEHAVIOR_ACTIVATE", "GEOMETRYARRAY_CHANGED",
			"MEDIA_CONTAINER_CHANGED", "RESIZE_CANVAS", "TOGGLE_CANVAS", "IMAGE_COMPONENT_CHANGED",
			"SCHEDULING_INTERVAL_CHANGED", "VIEWSPECIFICGROUP_CHANGED", "VIEWSPECIFICGROUP_INIT",
			"VIEWSPECIFICGROUP_CLEAR", "ORDERED_GROUP_TABLE_CHANGED", "BEHAVIOR_REEVALUATE", "CREATE_OFFSCREENBUFFER",
			"DESTROY_CTX_AND_OFFSCREENBUFFER", "SHADER_ATTRIBUTE_CHANGED", "SHADER_ATTRIBUTE_SET_CHANGED",
			"SHADER_APPEARANCE_CHANGED", "ALLOCATE_CANVASID", "FREE_CANVASID", };
	...
	/**
	 * Yields the current thread, by sleeping for a small amount of time. Unlike
	 * <code>Thread.yield()</code>, this method guarantees that the current thread
	 * will yield to another thread waiting to run. It also ensures that the other
	 * threads will run for at least a small amount of time before the current
	 * thread runs again.
	 */
	...
	private synchronized void logTimes() {
		...
		if (++frameCycleTick >= 10) {
			StringBuffer strBuf = new StringBuffer();
			strBuf.append("----------------------------------------------\n").append("    Frame Number = ")
					.append(frameCycleNumber).append("\n");
			for (int i = 0; i < statTimes.length; i++) {
				if (statSeen[i]) {
					strBuf.append("    ");
					if (statCounts[i] > 0) {
						strBuf.append(TimeType.values()[i]).append(" [").append(statCounts[i]).append("] = ")
						...
						strBuf.append(TimeType.values()[i]).append(" [0] = 0.0 msec\n");
					}
				}
			}
