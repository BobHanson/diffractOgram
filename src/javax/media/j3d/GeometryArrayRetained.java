...
/**
 * The GeometryArray object contains arrays of positional coordinates, colors,
 * normals and/or texture coordinates that describe point, line, or surface
 * geometry. It is extended to create the various primitive types (e.g., lines,
 * triangle_strips, etc.)
 */
...
// Array List used for messages
	...
	static final int targetThreads = (J3dThread.UPDATE_RENDER | J3dThread.UPDATE_GEOMETRY);
	...
	static final int VERTEX_CHANGED = COORDINATE_CHANGED | NORMAL_CHANGED | COLOR_CHANGED | TEXTURE_CHANGED
			| VATTR_CHANGED;
...
// above.
	...
	void setLive(boolean inBackgroundGroup, int refCount) {
		...
		if (this.refCount > 1) {
			...
			/*
			 * J3dMessage createMessage = new J3dMessage(); createMessage.threads =
			 * J3dThread.UPDATE_RENDERING_ATTRIBUTES; createMessage.type =
			 * J3dMessage.GEOMETRYARRAY_CHANGED; createMessage.universe = null;
			 * createMessage.args[0] = this; createMessage.args[1]= new
			 * Integer(DLIST_CREATE_CHANGED);
	...
	void disableGlobalAlpha(javax.media.j3d.Context c, boolean a, boolean b) {
		// ignore  bh needed
	}
	...
	void computeBoundingBox(float[] coords) {
		// System.err.println("GeometryArrayRetained : computeBoundingBox(float[]
		// coords)");
		int i, j, k, sIndex;
		double xmin, xmax, ymin, ymax, zmin, zmax;
	...
	void setupMirrorInterleavedColorPointer(boolean force) {
		...
		if (force || (c4fAllocated != 0)) { // Color is present
			...
			if ((vertexFormat & GeometryArray.USE_NIO_BUFFER) == 0 && interLeavedVertexData != null) { // java array
				if ((vertexFormat & GeometryArray.WITH_ALPHA) != 0) {

					for (i = initialVertexIndex; i < validVertexCount; i++) {
						mirrorInterleavedColorPointer[0][index++] = interLeavedVertexData[offset];
						mirrorInterleavedColorPointer[0][index++] = interLeavedVertexData[offset + 1];
						mirrorInterleavedColorPointer[0][index++] = interLeavedVertexData[offset + 2];
						mirrorInterleavedColorPointer[0][index++] = interLeavedVertexData[offset + 3];
						offset += stride;
					}
				} else {
					for (i = initialVertexIndex; i < validVertexCount; i++) {
				...
				if ((vertexFormat & GeometryArray.WITH_ALPHA) != 0 && interleavedFloatBufferImpl != null) {
					for (i = initialVertexIndex; i < validVertexCount; i++) {
						interleavedFloatBufferImpl.position(offset);
						interleavedFloatBufferImpl.get(mirrorInterleavedColorPointer[0], index, 4);
						index += 4;
						offset += stride;
					}
				} else {
					for (i = initialVertexIndex; i < validVertexCount; i++) {
						interleavedFloatBufferImpl.position(offset);
						interleavedFloatBufferImpl.get(mirrorInterleavedColorPointer[0], index, 3);
	...
	void setupMirrorColorPointer(int ctype, boolean force) {
		...
		if (c4fAllocated == 0 && !force) {
			...
			// only for RGB
			if (force && (c4fAllocated == 0) && (vertexFormat & GeometryArray.WITH_ALPHA) == 0) {
		...
		if ((vertexFormat & GeometryArray.USE_NIO_BUFFER) == 0) { // java array
			...
			case CF:
				if (floatRefColors == null) {
					if ((c4fAllocated == 0) && !force && (vertexType & COLOR_DEFINED) == CF) {
				...
				if (c4fAllocated == 0 && !force) {
					...
					if ((vertexFormat & GeometryArray.WITH_ALPHA) == 0) {
						...
						for (i = initialColorIndex; i < validVertexCount; i++) {
							mirrorFloatRefColors[0][dstIndex++] = floatRefColors[srcIndex++];
							mirrorFloatRefColors[0][dstIndex++] = floatRefColors[srcIndex++];
							mirrorFloatRefColors[0][dstIndex++] = floatRefColors[srcIndex++];
						...
						System.arraycopy(floatRefColors, srcIndex, mirrorFloatRefColors[0], srcIndex,
								(4 * validVertexCount));
					}
				}
				break;
			case CUB:
				if (byteRefColors == null) {
					if (c4fAllocated == 0 && !force && ((vertexType & COLOR_DEFINED) == CUB)) {
				...
				if (c4fAllocated == 0 && !force) {
					...
					if ((vertexFormat & GeometryArray.WITH_ALPHA) == 0) {
						...
						for (i = initialColorIndex; i < validVertexCount; i++) {
							mirrorUnsignedByteRefColors[0][dstIndex++] = byteRefColors[srcIndex++];
							mirrorUnsignedByteRefColors[0][dstIndex++] = byteRefColors[srcIndex++];
							mirrorUnsignedByteRefColors[0][dstIndex++] = byteRefColors[srcIndex++];
							mirrorUnsignedByteRefColors[0][dstIndex++] = (byte) (255.0);
						}
					} else {
						srcIndex = initialColorIndex * 4;
						System.arraycopy(byteRefColors, srcIndex, mirrorUnsignedByteRefColors[0], srcIndex,
								(4 * validVertexCount));
					}
				}
			...
			case C3UB:
				...
				if ((mirrorColorAllocated & CUB) == 0) {
					mirrorUnsignedByteRefColors[0] = new byte[vertexCount * multiplier];
					mirrorColorAllocated |= CUB;
				}
				if ((c4fAllocated & GeometryArray.WITH_ALPHA) == 0) {
					dstIndex = initialColorIndex * 3;
			...
			if (colorRefBuffer == null) {
				...
				if (c4fAllocated == 0 && !force) {
					...
					if ((vertexFormat & GeometryArray.WITH_ALPHA) == 0) {
						...
						for (i = initialColorIndex; i < validVertexCount; i++) {
							byteBufferRefColors.get(mirrorUnsignedByteRefColors[0], dstIndex, 3);
		...
		case T2F:
			...
			if (mirrorFloatRefTexCoords != null) {
				if (mirrorFloatRefTexCoords.length < (vertexCount * 2))
					mirrorRefTexCoords[texCoordSet] = mirrorFloatRefTexCoords = new float[vertexCount * 2];
			} else {
				mirrorRefTexCoords[texCoordSet] = mirrorFloatRefTexCoords = new float[vertexCount * 2];
			}
		...
		case T3F:
			...
			if (mirrorFloatRefTexCoords != null) {
				if (mirrorFloatRefTexCoords.length < (vertexCount * 3))
					mirrorRefTexCoords[texCoordSet] = mirrorFloatRefTexCoords = new float[vertexCount * 3];
			} else {
				mirrorRefTexCoords[texCoordSet] = mirrorFloatRefTexCoords = new float[vertexCount * 3];
			}
	...
	private void doSetupMirrorVertexAttrPointer(int vertexAttrNum, int type) {
		...
		case AF:
			vertexAttrType = AF;
			mirrorFloatRefVertexAttrs[vertexAttrNum] = floatRefVertexAttrs[vertexAttrNum];
			break;
		default:
			break;
		}
	...
	void createGeometryArrayData(int vertexCount, int vertexFormat) {
		if ((vertexFormat & GeometryArray.TEXTURE_COORDINATE) != 0) {
			createGeometryArrayData(vertexCount, vertexFormat, 1, defaultTexCoordSetMap);
		} else {
	...
	void createGeometryArrayData(int vertexCount, int vertexFormat, int texCoordSetCount, int[] texCoordSetMap) {
		...
		createGeometryArrayData(vertexCount, vertexFormat, texCoordSetCount, texCoordSetMap, 0, null);
	}
	...
	void setVertexFormat(boolean useAlpha, boolean ignoreVC, Context ctx) {
		Pipeline.getPipeline().setVertexFormat(ctx, this, vertexFormat, useAlpha, ignoreVC);
	}
	...
	float[] updateAlphaInFloatRefColors(Canvas3D cv, int screen, float alpha) {
		 ...
		 * lastAlpha[screen]);
		 * 
		 * System.err.println("((colorChanged & (1<<screen)) == 0) " + ((colorChanged &
		 * (1<<screen)) == 0));
		 */
		...
		if ((colorChanged & (1 << screen)) == 0) {
			...
			if (Math.abs(lastAlpha[screen] - alpha) <= EPSILON) {
				...
				// System.err.println("updateAlphaInFloatRefColors 0 : alpha is the same as the
				// last one " + alpha);
				...
				// System.err.println("updateAlphaInFloatRefColors 1 : alpha is different,
				// update the alpha values " + alpha);
			...
			if (screen == 0) {
				...
				// System.err.println("updateAlphaInFloatRefColors 2 : just update alpha = " +
				// alpha);
				...
				// System.err.println("updateAlphaInFloatRefColors 3 : update color values from
				// screen 0 data " + alpha);
	...
	byte[] updateAlphaInByteRefColors(Canvas3D cv, int screen, float alpha) {
		...
		if ((colorChanged & (1 << screen)) == 0) {
			...
			if (Math.abs(lastAlpha[screen] - alpha) <= EPSILON) {
				// and if alpha is the same as the last one,
				// just return the data
				// System.err.println("updateAlphaInByteRefColors 0 : alpha is the same as the
				// last one " + alpha);
				...
				// System.err.println("updateAlphaInByteRefColors 1 : alpha is different, update
				// the alpha values " + alpha);
			...
			// color data is modified
			if (screen == 0) {
				// System.err.println("updateAlphaInByteRefColors 2 : just update alpha =" +
				// alpha);
				...
				// System.err.println("updateAlphaInByteRefColors 3 : update color values from
				// screen 0 data " + alpha);
	...
	Object[] updateAlphaInVertexData(Canvas3D cv, int screen, float alpha) {
		...
		if (mvertexData == null || mvertexData.length <= screen) {
			...
			if (screen > 0) {
				for (int i = oldSize; i < screen + 1; i++) {
					...
					System.arraycopy(cfData[0], 0, cfData[i], 0, stride * vertexCount);
					lastAlpha[i] = lastAlpha[0];
				}
			}
		...
		if ((colorChanged & (1 << screen)) == 0) {
			...
			if (Math.abs(lastAlpha[screen] - alpha) <= EPSILON) {
				...
				for (int i = 0, j = colorOffset; i < vertexCount; i++, j += stride) {
					cdata[j + 3] *= m;
				}
			}
		} else {
	...
	void execute(Canvas3D cv, RenderAtom ra, boolean isNonUniformScale, boolean updateAlpha, float alpha, int screen,
		...
		if ((vertexFormat & GeometryArray.BY_REFERENCE) == 0) {
			...
			Pipeline.getPipeline().execute(cv.ctx, this, geoType, isNonUniformScale, useAlpha, ignoreVertexColors,
					initialVertexIndex, validVertexCount,
					((vertexFormat & GeometryArray.COLOR) != 0) ? (vertexFormat | GeometryArray.COLOR_4) : vertexFormat,
					texCoordSetCount, texCoordSetMap, (texCoordSetMap == null) ? 0 : texCoordSetMap.length,
					texCoordSetMapOffset, cv.numActiveTexUnit, vertexAttrCount, vertexAttrSizes, vdata, null, cdirty);
		}
		...
		// By reference with java array
			...
			if ((vertexFormat & GeometryArray.INTERLEAVED) != 0) {
				...
				Pipeline.getPipeline().execute(cv.ctx, this, geoType, isNonUniformScale, useAlpha, ignoreVertexColors,
						initialVertexIndex, validVertexCount, vertexFormat, texCoordSetCount, texCoordSetMap,
						(texCoordSetMap == null) ? 0 : texCoordSetMap.length, texCoordSetMapOffset, cv.numActiveTexUnit,
						vertexAttrCount, vertexAttrSizes, interLeavedVertexData, cdata, cdirty);
			...
			// non interleaved data
				...
				if ((vertexType == 0) || ((vertexType & VERTEX_DEFINED) == 0)
						|| (((vertexFormat & GeometryArray.COLOR) != 0) && (vertexType & COLOR_DEFINED) == 0)
						|| (((vertexFormat & GeometryArray.NORMALS) != 0) && (vertexType & NORMAL_DEFINED) == 0)
					...
					Pipeline.getPipeline().executeVA(cv.ctx, this, geoType, isNonUniformScale, ignoreVertexColors,
							validVertexCount, (vertexFormat | c4fAllocated), vdefined, initialCoordIndex,
							mirrorFloatRefCoords, mirrorDoubleRefCoords, initialColorIndex, cfdata, cbdata,
							initialNormalIndex, mirrorFloatRefNormals, vertexAttrCount, vertexAttrSizes,
							initialVertexAttrIndex, mirrorFloatRefVertexAttrs,
							((texCoordSetMap == null) ? 0 : texCoordSetMap.length), texCoordSetMap, cv.numActiveTexUnit,
							initialTexCoordIndex, texCoordStride, mirrorRefTexCoords, cdirty);
		...
		// By reference with nio buffer
			...
			if ((vertexFormat & GeometryArray.INTERLEAVED) != 0) {
				...
				Pipeline.getPipeline().executeInterleavedBuffer(cv.ctx, this, geoType, isNonUniformScale, useAlpha,
						ignoreVertexColors, initialVertexIndex, validVertexCount, vertexFormat, texCoordSetCount,
						texCoordSetMap, (texCoordSetMap == null) ? 0 : texCoordSetMap.length, texCoordSetMapOffset,
						cv.numActiveTexUnit, interleavedFloatBufferImpl, cdata, cdirty);
			...
			// non interleaved data
				...
				if ((vertexType == 0) || ((vertexType & VERTEX_DEFINED) == 0)
					...
					Pipeline.getPipeline().executeVABuffer(cv.ctx, this, geoType, isNonUniformScale, ignoreVertexColors,
							validVertexCount, (vertexFormat | c4fAllocated), vdefined, initialCoordIndex, vcoord,
							initialColorIndex, cdataBuffer, cfdata, cbdata, initialNormalIndex, normal, vertexAttrCount,
							vertexAttrSizes, initialVertexAttrIndex, floatBufferRefVertexAttrs,
							((texCoordSetMap == null) ? 0 : texCoordSetMap.length), texCoordSetMap, cv.numActiveTexUnit,
							initialTexCoordIndex, texCoordStride, refTexCoords, cdirty);
				} // end of all vertex data being set
			} // end of non interleaved case
		} // end of by reference with nio-buffer case
	}

	void buildGA(Canvas3D cv, RenderAtom ra, boolean isNonUniformScale, boolean updateAlpha, float alpha,
			boolean ignoreVertexColors, Transform3D xform, Transform3D nxform) {
		...
		if (vdata != null) {
			 ...
			 * System.err.println("geoType = "+geoType+" initialVertexIndex = "
			 * +initialVertexIndex+" validVertexCount = "
			 * +validVertexCount+" vertexFormat = "+vertexFormat+"  vertexData = "
			 * +vertexData);
			 */
			Pipeline.getPipeline().buildGA(cv.ctx, this, geoType, isNonUniformScale, updateAlpha, alpha,
					ignoreVertexColors, initialVertexIndex, validVertexCount, vertexFormat, texCoordSetCount,
					texCoordSetMap, (texCoordSetMap == null) ? 0 : texCoordSetMap.length, texCoordSetMapOffset,
					vertexAttrCount, vertexAttrSizes, (xform == null) ? null : xform.mat,
					(nxform == null) ? null : nxform.mat, vdata);
		} else {
			// Check if a vertexformat is set, but the array is null
			// if yes, don't draw anything
			if ((vertexType == 0) || ((vertexType & VERTEX_DEFINED) == 0)
					|| (((vertexFormat & GeometryArray.COLOR) != 0) && (vertexType & COLOR_DEFINED) == 0)
					|| (((vertexFormat & GeometryArray.NORMALS) != 0) && (vertexType & NORMAL_DEFINED) == 0)
					|| (((vertexFormat & GeometryArray.VERTEX_ATTRIBUTES) != 0) && (vertexType & VATTR_DEFINED) == 0)
					|| (((vertexFormat & GeometryArray.TEXTURE_COORDINATE) != 0)
							&& (vertexType & TEXCOORD_DEFINED) == 0)) {
			...
			if ((vertexFormat & GeometryArray.USE_NIO_BUFFER) == 0) {
				...
				Pipeline.getPipeline().buildGAForByRef(cv.ctx, this, geoType, isNonUniformScale, updateAlpha, alpha,
						ignoreVertexColors, validVertexCount, vertexFormat, vdefined, initialCoordIndex,
						mirrorFloatRefCoords, mirrorDoubleRefCoords, initialColorIndex, mirrorFloatRefColors[0],
						mirrorUnsignedByteRefColors[0], initialNormalIndex, mirrorFloatRefNormals, vertexAttrCount,
						vertexAttrSizes, initialVertexAttrIndex, mirrorFloatRefVertexAttrs,
						((texCoordSetMap == null) ? 0 : texCoordSetMap.length), texCoordSetMap, initialTexCoordIndex,
						texCoordStride, mirrorRefTexCoords, (xform == null) ? null : xform.mat,
						(nxform == null) ? null : nxform.mat);
			}
			/*
			 * // NOTE: NIO buffers are no longer supported in display lists. // This was
			 * never enabled by default anyway (only when the // optimizeForSpace property
			 * was set to false), so it wasn't // well-tested. If future support is desired,
			 * we will need to // add vertex attributes to buildGAForBuffer. There are no
			 * plans // to ever do this. else { // NIO Buffer case Object vcoord = null,
			 * cdataBuffer=null, normal=null;
			 * 
			 * int vdefined = 0; if((vertexType & PF) != 0) { vdefined |= COORD_FLOAT;
			 * vcoord = floatBufferRefCoords.getBufferAsObject(); } else if((vertexType & PD
			 * ) != 0) { vdefined |= COORD_DOUBLE; vcoord =
			 * doubleBufferRefCoords.getBufferAsObject(); }
			 * 
			 * if((vertexType & CF ) != 0) { vdefined |= COLOR_FLOAT; cdataBuffer =
			 * floatBufferRefColors.getBufferAsObject(); } else if((vertexType & CUB) != 0)
			 * { vdefined |= COLOR_BYTE; cdataBuffer =
			 * byteBufferRefColors.getBufferAsObject(); }
			 * 
			 * if((vertexType & NORMAL_DEFINED) != 0) { vdefined |= NORMAL_FLOAT; normal =
			 * floatBufferRefNormals.getBufferAsObject(); }
			 * 
			 * if((vertexType & TEXCOORD_DEFINED) != 0) vdefined |= TEXCOORD_FLOAT; // NOTE
			 * : need to add vertex attrs Pipeline.getPipeline().buildGAForBuffer(cv.ctx,
			 * this, geoType, isNonUniformScale, updateAlpha, alpha, ignoreVertexColors,
			 * validVertexCount, vertexFormat, vdefined, initialCoordIndex, vcoord,
			 * initialColorIndex,cdataBuffer, initialNormalIndex, normal, ((texCoordSetMap
			 * == null) ? 0:texCoordSetMap.length), texCoordSetMap,
			 * initialTexCoordIndex,texCoordStride, refTexCoords, (xform == null) ? null :
			 * xform.mat, (nxform == null) ? null : nxform.mat); }
			 */
		...
		if (((src.vertexFormat & GeometryArray.BY_REFERENCE) == 0)
			...
			for (index = start; index < end; index++) {
				if ((vertexFormat & GeometryArray.NORMALS) != 0) {
					System.arraycopy(vdata, src.indexNormal[index] * src.stride + src.normalOffset, vertexData,
							vOffset + normalOffset, 3);
				}
				if (colorStride == 4) {
					// System.err.println("===> copying color3");
					System.arraycopy(vdata, src.indexColor[index] * src.stride + src.colorOffset, vertexData,
							vOffset + colorOffset, colorStride);
				...
				if ((vertexFormat & GeometryArray.TEXTURE_COORDINATE) != 0) {
					...
					for (i = 0; i < texCoordSetCount; i++, tcOffset += texCoordStride) {
				...
				if ((vertexFormat & GeometryArray.VERTEX_ATTRIBUTES) != 0) {
					...
					for (i = 0; i < vertexAttrCount; i++) {
						...
						System.arraycopy(vdata, (src.indexVertexAttr[i][index]) * src.stride + src.vertexAttrOffsets[i],
				...
				if ((vertexFormat & GeometryArray.COORDINATES) != 0) {
					...
					System.arraycopy(vdata, src.indexCoord[index] * src.stride + src.coordinateOffset, vertexData,
			...
			if ((vertexFormat & GeometryArray.NORMALS) != 0) {
				...
				case NF:
					for (index = start; index < end; index++) {
						System.arraycopy(src.floatRefNormals, src.indexNormal[index] * 3, vertexData, vOffset, 3);
						vOffset += stride;
					}
					break;
				case N3F:
			...
			if ((vertexFormat & GeometryArray.COLOR) != 0) {
				...
				case CF:
					for (index = start; index < end; index++) {
						if ((src.vertexFormat & GeometryArray.WITH_ALPHA) != 0) {
							System.arraycopy(src.floatRefColors, src.indexColor[index] * multiplier, vertexData,
									vOffset, 4);
						} else {
							System.arraycopy(src.floatRefColors, src.indexColor[index] * multiplier, vertexData,
									vOffset, 3);
			...
			if ((vertexFormat & GeometryArray.TEXTURE_COORDINATE) != 0) {
				...
				case TF:
					for (index = start; index < end; index++) {
						for (i = 0, tOffset = vOffset; i < texCoordSetCount; i++) {
							System.arraycopy(src.refTexCoords[i], src.indexTexCoord[i][index] * texCoordStride,
				...
				case T2F:
					for (index = start; index < end; index++) {
						for (i = 0, tOffset = vOffset; i < texCoordSetCount; i++) {
							...
							vertexData[tOffset] = ((TexCoord2f[]) src.refTexCoords[i])[srcOffset].x;
							vertexData[tOffset + 1] = ((TexCoord2f[]) src.refTexCoords[i])[srcOffset].y;
							tOffset += texCoordStride;
						}
						vOffset += stride;
					}
				...
				case T3F:
					for (index = start; index < end; index++) {
						for (i = 0, tOffset = vOffset; i < texCoordSetCount; i++) {
							srcOffset = src.indexTexCoord[i][index];
							vertexData[tOffset] = ((TexCoord3f[]) src.refTexCoords[i])[srcOffset].x;
							vertexData[tOffset + 1] = ((TexCoord3f[]) src.refTexCoords[i])[srcOffset].y;
							vertexData[tOffset + 2] = ((TexCoord3f[]) src.refTexCoords[i])[srcOffset].z;
							tOffset += texCoordStride;
						}
						vOffset += stride;
					}
					...
					for (index = start; index < end; index++) {
						for (i = 0; i < vertexAttrCount; i++) {
							...
							System.arraycopy(src.floatRefVertexAttrs[i],
									src.indexVertexAttr[i][index] * vertexAttrSizes[i], vertexData, vaOffset,
									vertexAttrSizes[i]);
						}
						vOffset += stride;
					}
			...
			if ((vertexFormat & GeometryArray.COORDINATES) != 0) {
				...
				case PF:
					for (index = start; index < end; index++) {
						System.arraycopy(src.floatRefCoords, src.indexCoord[index] * 3, vertexData, vOffset, 3);
						vOffset += stride;
					}
					break;
				case PD:
...
//        System.err.println("unIndexifyNIOBuffer");
				...
				if ((vertexFormat & GeometryArray.TEXTURE_COORDINATE) != 0) {
					...
					for (i = 0; i < texCoordSetCount; i++, tcOffset += texCoordStride) {
						...
						src.interleavedFloatBufferImpl
								.position((src.indexTexCoord[i][index]) * src.stride + src.textureOffset);
			...
			if ((vertexFormat & GeometryArray.COLOR) != 0) {
				...
				case CUB:
					for (index = start; index < end; index++) {
						...
						vertexData[vOffset + 1] = (src.byteBufferRefColors.get(srcOffset + 1) & 0xff)
								* ByteToFloatScale;
						vertexData[vOffset + 2] = (src.byteBufferRefColors.get(srcOffset + 2) & 0xff)
								* ByteToFloatScale;

						if ((src.vertexFormat & GeometryArray.WITH_ALPHA) != 0) {
							vertexData[vOffset + 3] = (src.byteBufferRefColors.get(srcOffset + 3) & 0xff)
									* ByteToFloatScale;
						} else {
							vertexData[vOffset + 3] = 1.0f;
						}
	...
	/**
	 * Returns the vertex stride in numbers of floats as a function of the
	 * vertexFormat.
	 * 
	...
	/**
	 * Returns the stride of the set of vertex attributes. This is the sum of the
	 * sizes of each vertex attribute.
	 * 
	...
	/**
	 * Returns the offset in number of floats from the start of a vertex to each
	 * per-vertex vertex attribute.
	 * 
	...
	/**
	 * Returns the offset in number of floats from the start of a vertex to the
	 * per-vertex texture coordinate data. texture coordinate data always follows
	...
	/**
	 * Returns the offset in number of floats from the start of a vertex to the
	 * per-vertex color data. color data always follows texture data
	 * 
	...
	/**
	 * Returns the offset in number of floats from the start of a vertex to the
	 * per-vertex normal data. normal data always follows color data
	 * 
	...
	/**
	 * Returns the offset in number of floats from the start of a vertex to the per
	 ...
	 *
	 * @return the number of vertex attributes in this GeometryArray object
	 */
	 ...
	 *
	 * @param vertexAttrSizes an array that will receive a copy of the vertex
	 *                        attribute sizes array. The array must hold at least
	...
	/**
	 * Sets the coordinates associated with the vertices starting at the specified
	 * index.
	 * 
	 * @param index       the vertex index
	...
	/**
	 * Sets the coordinates associated with the vertices starting at the specified
	 * index for this object using coordinate data starting from vertex index
	 * <code>start</code> for <code>length</code> vertices.
	 * 
	 * @param index       the vertex index
	 * @param coordinates an array of vectors containing new coordinates
	...
	void setCoordinates(int index, float coordinates[], int start, int length) {
		...
		dirtyFlag |= COORDINATE_CHANGED;
		for (i = start * 3, j = offset; i < (start + length) * 3; i += 3, j += this.stride) {
			this.vertexData[j] = coordinates[i];
			this.vertexData[j + 1] = coordinates[i + 1];
			this.vertexData[j + 2] = coordinates[i + 2];
		}
	...
	/**
	 * Sets the colors associated with the vertices starting at the specified index
	 * for this object using data in <code>color</code>s starting at index
	 * <code>start</code> for <code>length</code> colors.
	 * 
	 * @param index  the vertex index
	 * @param colors an array of 3*n or 4*n values containing n new colors
	...
	void setColors(int index, float colors[], int start, int length) {
		...
		if ((this.vertexFormat & GeometryArray.WITH_ALPHA) != 0) {
			for (i = start * 4, j = offset; i < (start + length) * 4; i += 4, j += this.stride) {
				this.vertexData[j] = colors[i];
				this.vertexData[j + 1] = colors[i + 1];
				this.vertexData[j + 2] = colors[i + 2];
				this.vertexData[j + 3] = colors[i + 3] * lastAlpha[0];
	...
	/**
	 * Sets the normals associated with the vertices starting at the specified
	 * index.
	 * 
	 * @param index   the vertex index
	 ...
	 * for this object using data in <code>normals</code> starting at index
	...
	/**
	 * Sets the texture coordinates associated with the vertices starting at the
	 * specified index for this object using data in <code>texCoords</code> starting
	 * at index <code>start</code> and ending at index <code>start+length</code>.
	 * 
	 * @param index     the vertex index
	 * @param texCoords the new texture coordinates
	 * @param start     starting texture coordinate index of data in
	 *                  <code>texCoords</code> .
	...
	void setTextureCoordinates(int texCoordSet, int index, float texCoords[], int start, int length) {
		...
		int offset = this.stride * index + textureOffset + texCoordSet * texCoordStride;
		int i, j, k;
		boolean isLive = source != null && source.isLive();
		if (isLive) {
			geomLock.getLock();
		...
		if ((this.vertexFormat & GeometryArray.TEXTURE_COORDINATE_4) != 0) {
			for (i = start * 4, j = offset, k = 0; k < length; j += this.stride, k++) {
			...
			for (i = start * 3, j = offset, k = 0; k < length; j += this.stride, k++) {
				this.vertexData[j] = texCoords[i++];
				this.vertexData[j + 1] = texCoords[i++];
				this.vertexData[j + 2] = texCoords[i++];
			}
		} else {
			for (i = start * 2, j = offset, k = 0; k < length; j += this.stride, k++) {
				this.vertexData[j] = texCoords[i++];
				this.vertexData[j + 1] = texCoords[i++];
			}
		}
	...
	/**
	 * Sets the vertex attribute associated with the vertex at the specified index
	 * in the specified vertex attribute number for this object.
	 *
	...
	/**
	 * Sets the vertex attributes associated with the vertices starting at the
	 * specified index in the specified vertex attribute number for this object
	 * using data in <code>vertexAttrs</code> starting at index <code>start</code>
	 * and ending at index <code>start+length</code>.
	 *
	 * @param index       starting destination vertex index in this geometry array
	 * @param vertexAttrs source array of 1*n, 2*n, 3*n, or 4*n values containing n
	 ...
	 * @param vertexAttrNum vertex attribute number in this geometry array
	 * @param index         starting destination vertex index in this geometry array
	 * @param vertexAttrs   source array of Point2f objects containing new vertex
	 *                      attributes
	 * @param start         starting source vertex index in <code>vertexAttrs</code>
	 *                      array.
	 * @param length        number of vertex attributes to be copied.
	 ...
	 * @param vertexAttrNum vertex attribute number in this geometry array
	 * @param index         starting destination vertex index in this geometry array
	 * @param vertexAttrs   source array of Point3f objects containing new vertex
	 *                      attributes
	 * @param start         starting source vertex index in <code>vertexAttrs</code>
	 *                      array.
	 * @param length        number of vertex attributes to be copied.
	 ...
	 * @param vertexAttrNum vertex attribute number in this geometry array
	 * @param index         starting destination vertex index in this geometry array
	 * @param vertexAttrs   source array of Point4f objects containing new vertex
	 *                      attributes
	 * @param start         starting source vertex index in <code>vertexAttrs</code>
	 *                      array.
	 * @param length        number of vertex attributes to be copied.
	...
	/**
	 * Gets the coordinates associated with the vertices starting at the specified
	 * index.
	 * 
	 * @param index       the vertex index
	...
	/**
	 * Gets the normals associated with the vertices starting at the specified
	 * index.
	 * 
	 * @param index   the vertex index
	...
	/**
	 * Gets the texture co-ordinate associated with the vertex at the specified
	 * index.
	 * 
	 * @param index    the vertex index
	...
	/**
	 * Gets the texture co-ordinates associated with the vertices starting at the
	 * specified index.
	 * 
	 * @param index     the vertex index
	...
	/**
	 * Gets the vertex attribute associated with the vertex at the specified index
	 * in the specified vertex attribute number for this object.
	 */
	...
	/**
	 * Gets the vertex attributes associated with the vertices starting at the
	 * specified index in the specified vertex attribute number for this object.
	 */
	public void getVertexAttrs(int vertexAttrNum, int index, float[] vertexAttrs) {
		...
		for (i = 0, j = offset; ((i < vertexAttrs.length)
				&& (j < this.vertexData.length)); i += size, j += this.stride) {
			for (k = 0; k < size; k++) {
	...
	public void getVertexAttrs(int vertexAttrNum, int index, Point2f[] vertexAttrs) {
		...
		for (i = 0, j = offset; ((i < vertexAttrs.length) && (j < this.vertexData.length)); i++, j += this.stride) {
	...
	boolean intersectBoundingBox(Point3d coordinates[], BoundingBox box, double dist[], Point3d iPnt) {
		...
		if ((out[0] == coordinates.length) || (out[1] == coordinates.length) || (out[2] == coordinates.length)
				|| (out[3] == coordinates.length) || (out[4] == coordinates.length) || (out[5] == coordinates.length))
	...
	boolean intersectBoundingSphere(Point3d coordinates[], BoundingSphere sphere, double dist[], Point3d iPnt) {
		...
		for (i = 0; i < coordinates.length; i++) {
			...
			if (tempV3D.length() <= sphere.radius) {
				...
				if (dist != null) {
					computeMinDistance(coordinates, sphere.getCenter(), null, dist, iPnt);
				}
		...
		for (i = 0; i < coordinates.length; i++) {
			if (i < (coordinates.length - 1))
				esFlag = edgeIntersectSphere(sphere, coordinates[i], coordinates[i + 1]);
			else
				esFlag = edgeIntersectSphere(sphere, coordinates[i], coordinates[0]);
			if (esFlag == true) {
				if (dist != null) {
					computeMinDistance(coordinates, sphere.getCenter(), null, dist, iPnt);
				}
		...
		/*
		 * for(i=0; i<coordinates.length; i++) System.err.println("coordinates P" + i +
		 * " " + coordinates[i]); for(i=0; i<coord2.length; i++)
		...
		if (pointIntersectPolygon2D(pNrm, coordinates, q)) {
			if (dist != null) {
				computeMinDistance(coordinates, sphere.getCenter(), pNrm, dist, iPnt);
			}
			return true;
		}
		return false;
	...
	boolean intersectBoundingPolytope(Point3d coordinates[], BoundingPolytope polytope, double dist[], Point3d iPnt) {
		...
		if (PreTest) {
			...
			for (int i = 0; i < coordinates.length; i++) {
				for (int j = 0; j < polytope.planes.length; j++) {
					if ((polytope.planes[j].x * coordinates[i].x + polytope.planes[j].y * coordinates[i].y
							+ polytope.planes[j].z * coordinates[i].z) <= (distanceSign) * polytope.planes[j].w) {
				...
				if (intersection) {
					...
					if (dist != null) {
						computeMinDistance(coordinates, polytope.getCenter(), null, dist, iPnt);
					}
					return true;
				}
			}
		...
		for (int i = 0; i < polytope.planes.length; i++) {
			for (int j = 0; j < coordinates.length; j++) {
				problemTableau[j][i] = (-1.0) * (polytope.planes[i].x * coordinates[j].x
						+ polytope.planes[i].y * coordinates[j].y + polytope.planes[i].z * coordinates[j].z);
			}
		}
		...
		double distance = generalStandardSimplexSolver(problemTableau, Float.NEGATIVE_INFINITY);
		if (debug) {
		...
		for (j = 0; j < coord.length; j++) {
			if (j < (coord.length - 1))
				if (det2D(coord2D[j], coord2D[j + 1], pnt) > 0.0)
					...
					return false;
			else if (det2D(coord2D[j], coord2D[0], pnt) > 0.0)
				;
			else
				return false;
		}
	...
	boolean edgeIntersectPlane(Vector3d normal, Point3d pnt, Point3d start, Point3d end, Point3d iPnt) {
		...
		if ((tr < 0.0) || (tr > 1.0)) {
			// System.err.println("Edge intersects the plane behind the start or exceed
			// end.");
			return false;
		}
	 ...
	 * Moller's code for fast triangle-triangle intersection from his "Real-Time
	 * Rendering" book.
	 *
	 * The code is now divisionless. It tests for separating by planes parallel to
	 * either triangle. If neither separate the triangles, then two cases are
	 * considered. First case is if the normals to the triangles are parallel. In
	 * that case, the triangles are coplanar and a sequence of tests are made to see
	 * if edges of each triangle intersect the other triangle. If the normals are
	 * not parallel, then the two triangles can intersect only on the line of
	 * intersection of the two planes. The intervals of intersection of the
	 * triangles with the line of intersection of the two planes are computed and
	 * tested for overlap.
	 */
	...
	boolean intersectPolygon(Point3d coord1[], Point3d coord2[]) {
		...
		/*
		 * for(i=0; i<coord1.length; i++) System.err.println("coord1 P" + i + " " +
		 * coord1[i]); for(i=0; i<coord2.length; i++) System.err.println("coord2 P" + i
		 * + " " + coord2[i]);
		 */
		...
		for (i = 0; i < coord2.length; i++) {
			if (i < (coord2.length - 1))
				epFlag = edgeIntersectPlane(pNrm, coord1[0], coord2[i], coord2[i + 1], seg[j]);
			else
				epFlag = edgeIntersectPlane(pNrm, coord1[0], coord2[i], coord2[0], seg[j]);
			if (epFlag) {
				if (++j > 1) {
					break;
				}
	 ...
	 * stored in dist[0] and the intersect point in iPnt (if iPnt is not null).
	 */
	boolean intersectRay(Point3d coordinates[], PickRay ray, double dist[], Point3d iPnt) {

		return intersectRayOrSegment(coordinates, ray.direction, ray.origin, dist, iPnt, false);
	 ...
	 * stored in pr.
	 */
	boolean intersectRayOrSegment(Point3d coordinates[], Vector3d direction, Point3d origin, double dist[],
			Point3d iPnt, boolean isSegment) {
		...
		if ((vec1.length() == 0) || (pNrm.length() == 0)) {
			...
			isIntersect = intersectLineAndRay(coordinates[l], coordinates[k], origin, direction, dist, iPnt);
		...
		if (pNrmDotrDir == 0.0) {
			...
			for (i = 0; i < coordinates.length; i++) {
				...
				if (intersectLineAndRay(coordinates[i], coordinates[k], origin, direction, dist, iPnt)) {
					isIntersect = true;
					break;
				}
			}
		...
		if ((dist[0] < -EPS) || (isSegment && (dist[0] > 1.0 + EPS))) {
			// Ray intersects the plane behind the ray's origin
		...
		if (absNrmX > absNrmY) {
			if (absNrmX < absNrmZ) {
				for (i = 0; i < coordinates.length; i++) {
					...
					sign = (iPnt.y - p0.y) * (p1.x - p0.x) - (iPnt.x - p0.x) * (p1.y - p0.y);
					if (isNonZero(sign)) {
						if (sign * lastSign < 0) {
							isIntersect = false;
							break;
				...
				for (i = 0; i < coordinates.length; i++) {
					p0 = coordinates[i];
					p1 = (i != coordinates.length - 1 ? coordinates[i + 1] : coordinates[0]);
					sign = (iPnt.y - p0.y) * (p1.z - p0.z) - (iPnt.z - p0.z) * (p1.y - p0.y);
					if (isNonZero(sign)) {
						if (sign * lastSign < 0) {
							isIntersect = false;
							break;
			...
			if (absNrmY < absNrmZ) {
				...
				for (i = 0; i < coordinates.length; i++) {
					p0 = coordinates[i];
					p1 = (i != coordinates.length - 1 ? coordinates[i + 1] : coordinates[0]);
					sign = (iPnt.x - p0.x) * (p1.z - p0.z) - (iPnt.z - p0.z) * (p1.x - p0.x);
					if (isNonZero(sign)) {
						if (sign * lastSign < 0) {
							isIntersect = false;
							break;
	...
	boolean inside(Point3d coordinates[], PickPoint point, int ccw) {
		 ...
		 * System.err.println("Triangle/Quad :"); for(i=0; i<coordinates.length; i++)
	...
	/**
	 * Return true if triangle or quad intersects with cylinder. The distance is
	 * stored in dist.
	 */
	boolean intersectCylinder(Point3d coordinates[], PickCylinder cyl, double dist[], Point3d iPnt) {
		...
		if (coordinates.length > 2) {
			if (cyl instanceof PickCylinderRay) {
				if (intersectRay(coordinates, new PickRay(origin, direction), dist, iPnt)) {
					return true;
				}
			} else {
		...
		for (int i = 0; i < coordinates.length; i++) {
			...
			if (cyl instanceof PickCylinderSegment) {
				sqDistToEdge = Utils.segmentToSegment(origin, end, coordinates[i], coordinates[j], iPnt1, iPnt, null);
			} else {
				sqDistToEdge = Utils.rayToSegment(origin, direction, coordinates[i], coordinates[j], iPnt1, iPnt, null);
			}
	...
	/**
	 * Return true if triangle or quad intersects with cone. The distance is stored
	 * in dist.
	 */
	...
	/**
	 * Return true if point intersects with cylinder and the distance is stored in
	 * dist.
	 */
	...
	void setCoordRefFloat(float[] coords) {
		...
		if (coords != null) {
			if ((vertexType & VERTEX_DEFINED) != 0 && (vertexType & VERTEX_DEFINED) != PF) {
	...
	void setCoordRefDouble(double[] coords) {
		...
		if (coords != null) {
			if ((vertexType & VERTEX_DEFINED) != 0 && (vertexType & VERTEX_DEFINED) != PD) {
				throw new IllegalArgumentException(J3dI18N.getString("GeometryArray98"));
			}
	...
	void setCoordRef3f(Point3f[] coords) {
		...
		if (coords != null) {
			if ((vertexType & VERTEX_DEFINED) != 0 && (vertexType & VERTEX_DEFINED) != P3F) {
				throw new IllegalArgumentException(J3dI18N.getString("GeometryArray98"));
			}
	...
	void setCoordRef3d(Point3d[] coords) {
		...
		if (coords != null) {
			if ((vertexType & VERTEX_DEFINED) != 0 && (vertexType & VERTEX_DEFINED) != P3D) {
				throw new IllegalArgumentException(J3dI18N.getString("GeometryArray98"));
			}
	...
	void setColorRefFloat(float[] colors) {
		...
		if (colors != null) {
			if ((vertexType & COLOR_DEFINED) != 0 && (vertexType & COLOR_DEFINED) != CF) {
				throw new IllegalArgumentException(J3dI18N.getString("GeometryArray98"));
			}
	...
	void setColorRefBuffer(J3DBuffer colors) {
		if (colors != null) {
			...
			} else if (colors.getROBuffer().limit() < getColorStride() * (initialColorIndex + validVertexCount)) {
				throw new ArrayIndexOutOfBoundsException(J3dI18N.getString("GeometryArray112"));
			}
		}
	...
	void setColorRefByte(byte[] colors) {
		...
		if (colors != null) {
			if ((vertexType & COLOR_DEFINED) != 0 && (vertexType & COLOR_DEFINED) != CUB) {
				throw new IllegalArgumentException(J3dI18N.getString("GeometryArray98"));
			}
	...
	void setColorRef3f(Color3f[] colors) {
		...
		if (colors != null) {
			if ((vertexType & COLOR_DEFINED) != 0 && (vertexType & COLOR_DEFINED) != C3F) {
				throw new IllegalArgumentException(J3dI18N.getString("GeometryArray98"));
			}
	...
	void setColorRef4f(Color4f[] colors) {
		...
		if (colors != null) {
			if ((vertexType & COLOR_DEFINED) != 0 && (vertexType & COLOR_DEFINED) != C4F) {
				throw new IllegalArgumentException(J3dI18N.getString("GeometryArray98"));
			}
	...
	void setColorRef3b(Color3b[] colors) {
		...
		if (colors != null) {
			...
			if ((vertexType & COLOR_DEFINED) != 0 && (vertexType & COLOR_DEFINED) != C3UB) {
				throw new IllegalArgumentException(J3dI18N.getString("GeometryArray98"));
			}
	...
	void setColorRef4b(Color4b[] colors) {
		...
		if (colors != null) {
			if ((vertexType & COLOR_DEFINED) != 0 && (vertexType & COLOR_DEFINED) != C4UB) {
				throw new IllegalArgumentException(J3dI18N.getString("GeometryArray98"));
			}
	...
	void setNormalRefFloat(float[] normals) {
		...
		if (normals != null) {
			if ((vertexType & NORMAL_DEFINED) != 0 && (vertexType & NORMAL_DEFINED) != NF) {
				throw new IllegalArgumentException(J3dI18N.getString("GeometryArray98"));
			}
	...
	void setNormalRef3f(Vector3f[] normals) {
		...
		if (normals != null) {
			if ((vertexType & NORMAL_DEFINED) != 0 && (vertexType & NORMAL_DEFINED) != N3F) {
				throw new IllegalArgumentException(J3dI18N.getString("GeometryArray98"));
			}
	...
	/**
	 * Sets the float vertex attribute array reference for the specified vertex
	 * attribute number to the specified array.
	 */
	void setVertexAttrRefFloat(int vertexAttrNum, float[] vertexAttrs) {
		...
		/*
		 * if (vertexAttrType != 0 && vertexAttrType != AF) { if (vertexAttrs != null) {
		 * // XXXX: new exception string throw new IllegalArgumentException(
		 * J3dI18N.getString("GeometryArray98-XXX")); } return; }
		 */
	...
	/**
	 * Gets the float vertex attribute array reference for the specified vertex
	 * attribute number.
	 */
	...
	/**
	 * Sets the vertex attribute buffer reference for the specified vertex attribute
	 * number to the specified buffer object.
	 */
	...
	/**
	 * Gets the vertex attribute array buffer reference for the specified vertex
	 * attribute number.
	 */
	...
	void setInterleavedVertexBuffer(J3DBuffer vertexData) {
		...
		if (vertexData != null) {
			...
			if (this instanceof IndexedGeometryArrayRetained) {
				...
				if (((this.vertexFormat & GeometryArray.COLOR) != 0) && (stride * idx.maxColorIndex >= bufferSize)) {
					throw new ArrayIndexOutOfBoundsException(J3dI18N.getString("IndexedGeometryArray24"));
				}

				if (((this.vertexFormat & GeometryArray.NORMALS) != 0) && (stride * idx.maxNormalIndex >= bufferSize)) {
					throw new ArrayIndexOutOfBoundsException(J3dI18N.getString("IndexedGeometryArray23"));
				}
			} else {
		...
		if ((vertexFormat & GeometryArray.INTERLEAVED) != 0) {
			...
			if ((vertexFormat & GeometryArray.USE_NIO_BUFFER) != 0) {
				...
				case AF:
					for (int i = 0; i < vertexAttrCount; i++) {
						...
						if (floatBufferRefVertexAttrs[i]
								.limit() < (sz * (initialVertexAttrIndex[i] + validVertexCount))) {
							throw new ArrayIndexOutOfBoundsException(J3dI18N.getString("GeometryArray129"));
						}
					}
					break;
			...
			// By reference with java array
					...
					for (int i = 0; i < texCoordSetCount; i++) {
						if ((vertexFormat & GeometryArray.TEXTURE_COORDINATE_2) != 0) {
							...
							if (((float[]) refTexCoords[i]).length < 3 * (initialTexCoordIndex[i] + validVertexCount)) {
								...
								if (((float[]) refTexCoords[i]).length < 4
										* (initialTexCoordIndex[i] + validVertexCount)) {
									throw new ArrayIndexOutOfBoundsException(J3dI18N.getString("GeometryArray113"));
								}
							}
						}
				...
				case AF:
					for (int i = 0; i < vertexAttrCount; i++) {
						int sz = vertexAttrSizes[i];
						if (floatRefVertexAttrs[i].length < (sz * (initialVertexAttrIndex[i] + validVertexCount))) {
							throw new ArrayIndexOutOfBoundsException(J3dI18N.getString("GeometryArray129"));
						}
					}
					break;
	...
	/**
	 * Sets the initial vertex attribute index for the specified vertex attribute
	 * number for this GeometryArray object.
	 */
	...
	/**
	 * Gets the initial vertex attribute index for the specified vertex attribute
	 * number for this GeometryArray object.
	 */
...
// This method is used for debugging only
	...
	/*
	 * // Ununsed int getVertexCount(int index) { return compileVcount[index]; }
	 * 
	 * 
	...
	boolean isMergeable() {
		...
		if ((vertexFormat & GeometryArray.TEXTURE_COORDINATE) != 0
				&& (texCoordSetCount > 1 || texCoordSetMap != null && texCoordSetMap.length > 1)) {
			return false;
		}
	...
	void initMirrorGeometry() {
		...
		if (this instanceof IndexedGeometryArrayRetained) {
			if ((vertexFormat & GeometryArray.USE_COORD_INDEX_ONLY) == 0) {
				mirrorGeometry = ((IndexedGeometryArrayRetained) this).cloneNonIndexedGeometry();
			} else {
				mirrorGeometry = null;
			}
		}
	...
	void computeMinDistance(Point3d coordinates[], Point3d center, Vector3d normal, double dist[], Point3d iPnt) {
		...
		if (coordinates.length == 3) {
			...
			if (normal != null) {
				...
				if (pointInTri(iPnt, coordinates[0], coordinates[1], coordinates[2], normal)) {
					return;
				}
			}
		...
		if (normal != null) {
			...
			if (pointInTri(iPnt, coordinates[0], coordinates[1], coordinates[2], normal)
					|| pointInTri(iPnt, coordinates[1], coordinates[2], coordinates[3], normal)) {
				return;
			}
		}
	...
	void handleFrequencyChange(int bit) {
		...
		if ((vertexFormat & GeometryArray.BY_REFERENCE) == 0) {
			if ((bit == GeometryArray.ALLOW_COORDINATE_WRITE)
					|| (((vertexFormat & GeometryArray.COLOR) != 0) && bit == GeometryArray.ALLOW_COLOR_WRITE)
					|| (((vertexFormat & GeometryArray.NORMALS) != 0) && bit == GeometryArray.ALLOW_NORMAL_WRITE)
					|| (((vertexFormat & GeometryArray.TEXTURE_COORDINATE) != 0)
