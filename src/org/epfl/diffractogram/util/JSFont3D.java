...
// Map glyph code to GeometryArrayRetained
	...
	/**
	 * Constructs a Font3D object from the specified Font and FontExtrusion objects,
	 * using the default value for the tessellation tolerance. The default value is
	 * as follows:
	 *
	 * <ul>
	 ...
	 * </ul>
	 * <P>
	 * The FontExtrusion object contains the extrusion path to use on the 2D Font
	 * glyphs. To ensure correct rendering the font must be created with the default
	 * AffineTransform. Passing null for the FontExtrusion parameter results in no
	 * extrusion being done.
	 *
	 * @param font        the Java 2D font used to create the 3D font object
	 * @param extrudePath the extrusion path used to describe how the edge of the
	 *                    font varies along the Z axis
	 */
	public JSFont3D(Font font, FontExtrusion extrudePath) {
		this(font, 0.01, extrudePath);
	}

	/**
	 * Constructs a Font3D object from the specified Font and FontExtrusion objects,
	 * using the specified tessellation tolerance. The FontExtrusion object contains
	 * the extrusion path to use on the 2D Font glyphs. To ensure correct rendering,
	 * the font must be created with the default AffineTransform. Passing null for
	 * the FontExtrusion parameter results in no extrusion being done.
	 *
	 * @param font                  the Java 2D font used to create the 3D font
	 *                              object.
	 * @param tessellationTolerance the tessellation tolerance value used in
	 *                              tessellating the glyphs of the 2D Font. This
	 *                              corresponds to the <code>flatness</code>
	 *                              parameter in the
	 *                              <code>java.awt.Shape.getPathIterator</code>
	 *                              method.
	 * @param extrudePath           the extrusion path used to describe how the edge
	 *                              of the font varies along the Z axis.
	 *
	 * @since Java 3D 1.2
	 * 
	 *
	 */
	public JSFont3D(Font font, double tessellationTolerance, FontExtrusion extrudePath) {
		this.font = font;
	 ...
	 * specified parameter.
	 *
	 * @param extrudePath object that will receive the FontExtrusion information for
	 *                    this Font3D object
	 */
	...
	public GeometryArray getGlyphGeometry(char c) {
		...
		// Assume that triangulateGlyphs returns a triangle array with only coords &
		// normals
	...
	GeometryArrayRetained triangulateGlyphs(GlyphVector gv, char c) {
		...
		if (geo == null) {
			...
			int vertOffset = 0;//gs.triangulateIslands(islandCounts, outVerts, contourCounts, triangData);
